...
class system:
    def __init__(self, sp, **kwargs):
        ...
        self.lamp = lamp(sp.lamp, app=app, gui=win)
            ...
            while(True): #use while loop to be able to break out at any time if run aborted.
                ...
                if len(v_smu) > 1:
                    ...
                    if IV_param['light_int'] > 0 and not self.abortRunFlag(): #don't analyze dark or aborted runs
                        ...
                        jv_metrics = bric_jv.get_metrics(df, False)
                        #print(jv_metrics)
    ...
    def generate_JV_Results_PDF(self, data, uname, IV_Results, dataFileName, pdfFilePath):
        ...
        ax1.invert_yaxis()
        ax1.set_ylabel("normalized current density [mA/$cm^2$]")
        ...
        if 'Jsc' in IV_Results:
            results_text.append(("Jsc @ " + nominalLightIntensityString,"{:5.3f}".format(IV_Results['Jsc']), " mA/$cm^2$"))
        if 'Voc' in IV_Results:
            results_text.append(("Voc @ " + nominalLightIntensityString,"{:6.4f}".format(IV_Results['Voc']),"V"))
        if 'FF' in IV_Results:
            results_text.append(("FF @ " + nominalLightIntensityString,"{:6.4f}".format(IV_Results['FF']),""))
        if 'Pce' in IV_Results:
            results_text.append(("Pce @ " + nominalLightIntensityString,"{:5.2f}".format(IV_Results['Pce']),"%"))
        if 'Jmpp' in IV_Results:
            results_text.append(("Jmpp @ " + nominalLightIntensityString,"{:5.3f}".format(IV_Results['Jmpp'])," mA/$cm^2$"))
        if 'Vmpp' in IV_Results:
            results_text.append(("Vmpp @ " + nominalLightIntensityString,"{:6.4f}".format(IV_Results['Vmpp']),"V"))
        if 'Pmpp' in IV_Results:
            results_text.append(("Pmpp @ " + nominalLightIntensityString,"{:7.3f}".format(IV_Results['Pmpp'])," mW/$cm^2$"))
...
if __name__ == "__main__":
    ...
    s.SMU.useReferenceDiode = False #True
    ...
    s.SMU.referenceDiodeImax = 0.005
